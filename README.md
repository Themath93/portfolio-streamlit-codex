...
1. **🏠 홈** - 환영 메시지, 주요 경력, 노션 포트폴리오 링크, 작업 하이라이트 이미지, LangChain 챗봇 인터페이스(추천 질문 제공)
...
- **추천 질문 빠른 시작**: 홈 챗봇에서 "이 지원자에 대해 설명해줘" 등 3가지 추천 질문 버튼으로 대화를 즉시 시작
- **노션 포트폴리오 연동**: `portfolio_data.json`의 `social_links.notion` 값을 통해 홈 화면에서 노션 상세 페이지 바로가기 제공
- **이미지 하이라이트 섹션**: `assets/projects/images` 폴더에 PNG 파일을 추가하면 홈 화면에서 자동으로 노출되어 최근 작업을 시각적으로 소개
