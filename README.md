...
- **한국어 홈 레이아웃**: 사이드바에 프로필 이미지·연락처·소셜 링크를 배치하고 본문에 자기소개, 학력·경력, 도메인 기반 기술 섹션을 한국어로 제공
- **도메인별 기술 그룹**: 데이터 엔지니어링·백엔드·데브옵스 3개 영역으로 기술 스택을 정리해 한눈에 파악 가능
...
- `skills.domains`에 각 영역별 기술 목록을 입력하면 홈 화면에서 데이터 엔지니어링·백엔드·데브옵스 카테고리로 자동 분류합니다. 해당 키가 없을 경우 `languages`/`frameworks`/`tools` 정보를 기반으로 그룹을 생성합니다.
...
- 홈 화면은 Streamlit 기본 컴포넌트를 활용해 다크/라이트 테마에 따라 자동으로 색상을 조정하며, 추가 CSS 없이도 가독성을 유지합니다.
